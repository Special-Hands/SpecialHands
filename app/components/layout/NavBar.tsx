...
export default function NavBar({ transparent = true, slideOut = false }: NavProps) {
  const items = [["OUR SERVICES", '/#OURSERVICES'],["ABOUT US", '/#ABOUTUS'], ["CONTACT US", '/#CONTACTUS'], ['BLOG', '/']];
  ...
  return (
    ...
    > 
      <nav  className="flex  duration-400 justify-between px-3  py-[0.5rem] pl-[2rem] p-4 items-center w-screen ">
        ...
        <ul className=" gap-7 pr-7  nav:pr-0 nav:hidden  flex text-xl items-center flex-grow justify-end">
          {items.map((item) => {
            return (
              <Link href={item[1]}>
              <li key={item[0].split(' ').join()} className="  text-[1.3rem] cursor-pointer  hover:border-b-[0.5rem] hover:border-[orange] hover:text-[orange] transition-all duration-300">
                {item[0]}
              </li>
              </Link>
            );
